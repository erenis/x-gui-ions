...
"""
Now that something is happening, lets get the GUI a little more interactive.

As of now, the time on the gui is not updating, and none of the buttons 
such as fast forward will do anything.

At the beginning of step(), add:

pc.getEvents(s1)
pc.setTime(s1.ntime*s1.dt)
pc.fastForward(s1.ntime*s1.dt, s1)

getEvents(s1) will handle events from the gui.  setTime sets the time on
on the control bar.  And fastForward listens to the fastForward comannd. 
When you type something in fastforward and hit enter, it sends a variable
change request to your code.  So we need to define an event handle.  We do this
with:

#define a callback.  This is rather standard
...
changeVarsCallback recieves a dictionary called "to", containg key-value pairs
of variable names (the key) to change to the corresponding values. Here we take advantage
of the dynamic nature of python.  We can set attributes of an object by name in ASCII form,
using setattr.

rightType takes a python number, and returns the proper numpy type required to interface with 
Fortran.  For this function, we import a new module: from types import *

Just for fun, lets do something with the reset button.  There is no generic way to reset the
simulation.  But we can lay out some general ideas:
...
This code will execute the callback when getEvents is called at the top of the step function.

Note that I had to define a new Fortran function to close the namelist number.
...
The reset as is does not quite work.  While init handles resetting most of the initial state,
more work is done.  For example, we need a different function to reset the RNG seed.

Also we need to restart the ntime loop.

It may not be obvious, however YOUR code, not the gui code, will handle all the events.  This
happens in the getEvents() call.  This means there is a time, delay.  If you press fast-forward,
or reset, the command will not execute until getEvents() is called at the beginning of the loop.
If the simulation is paused, nothing will happen until step() is run again.

The reason for this is to allow code to interface with the GUI in a predictable way.  The user
can control when event processing can happen.  So if the GUI requests a change to state variables,
you can ensure this does not happen at an inconvenient spot in the middle of the loop.

Lets do one more thing is this tutorial, read the time reverse box.  When the box toggles,
the gui will request a variable change to s1.timedirction, and toggle it to 1 (reverse) or
0 (forward).  First, in init, we initialize  "s1.timedirection = 0".  Now we can monitor
the time direction, right after getEvents call "print s1.timedirection"
