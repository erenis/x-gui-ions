...
if (in1.nextrand > 0):
   minit1.mnextran1(in1.nextrand,ndim,np+npi)
...
# open file: updates nderec and possibly iude
   if (in1.nderec==0):
      mdiag1.dafopennc1(denet,iude,in1.nderec,fdename)
...
# open file: updates ndirec and possibly iudi
   if (in1.ndirec==0):
      mdiag1.dafopennc1(denit,iudi,in1.ndirec,fdiname)
...
# open file: updates nprec and possibly iup
   if (in1.nprec==0):
      mdiag1.dafopennc1(pott,iup,in1.nprec,fpname)
# spectral analysis
...
# open file: updates nelrec and possibly iuel
   if (in1.nelrec==0):
      mdiag1.dafopennc1(elt,iuel,in1.nelrec,felname)

# initialize velocity diagnostic
if (in1.ntv > 0):
# sfv = electron velocity distribution functions in tile
   sfv = numpy.empty((2*in1.nmv+2,1,mx1+1),float_type,'F')
...
# fvtp = velocity distribution function for test particles
      fvtp = numpy.empty((2*in1.nmv+2,1),float_type,'F')
# fvmtp = vdrift, vth, and entropy for test particles
      fvmtp = numpy.empty((3,1),float_type,'F')
      fvtp[0,:] = 2.0*int(max(4.0*in1.vtx+abs(in1.vx0),
                              4.0*in1.vtdx+abs(in1.vdx)))
...
# display smoothed ion density
               graf1.dscaler1(sfield,' IDENSITY',ntime,999,1,nx,irc)
               if (irc[0]==1):
                  break
               irc[0] = 0
# ion spectral analysis
               if ((in1.nddi==2) or (in1.nddi==3)):
                  ...
                  mdiag1.micspect1(denit,wmi,pkwdi,pksdi,ts,in1.t0,
                                   tdiag,mtdi,iwi,in1.modesxdi,nx,-1)
# performs frequency analysis of accumulated complex time series
                  wkdi[:,0] = wm[numpy.argmax(pkwdi[:,:,0],axis=1)]
                  wkdi[:,1] = wm[numpy.argmax(pkwdi[:,:,1],axis=1)]
# display frequency spectrum
                  graf1.dmscaler1(wkdi,'IDENSITY OMEGA VS MODE',ntime,
...
# display frequency spectrum
            graf1.dmscaler1(wk,'POTENTIAL OMEGA VS MODE',ntime,999,1,
                            in1.modesxp,cwk,irc)
            if (irc[0]==1):
               break
            irc[0] = 0
...
if (in1.ntt > 0):
   if ((in1.nst==1) or (in1.nst==2)):
      ...
      graf1.displayt1(partd,ts,in1.dt*float(in1.ntt),itt,2,3,irc)
      if (irc[0]==1):
         exit(1)
...
# performs frequency analysis of accumulated complex time series
      ...
      wkdi[:,1] = wm[numpy.argmax(pkwdi[:,:,1],axis=1)]
# display frequency spectrum
      graf1.dmscaler1(wkdi,'IDENSITY OMEGA VS MODE',ntime,999,1,
