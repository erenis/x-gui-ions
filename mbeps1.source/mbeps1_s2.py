...
"""
This file is step 2.  We begin from mbeps1_s1.py.   First we need to copy the gui files in to a folder called gui and import them.
We do this with the lines:

import sys
sys.path.append('./gui')
from ProceduralInterface import *

We then initialize the gui.  This is done in 3 lines:
...
The thrid line should always have 0 as a paremeter, unless you know what you are doing.  OPTIONAL KNOWLEDGE:
the line pc.asyncMode() command changes the mode between synchronous and asyncronous communication with the gui.
In async mode, when you plot data, the plot command will not wait for the graph to draw.  This is not ideal,
as the gui rendering will run slower than the simulation, and interactivity will not be possible.  With 
fast forwarding, the slow gui performance should not be an issue either.

At the end of the simulation, we call pc.wait(), which simply maintains a loop to keep the GUI interactive
when the simulation is over

Currently, the code will do absolutely nothing, other than create a GUI that displays some blank graphs.
...
This will display the electron velocity distribution.  For the curious, I will talk a little about the technical details.


The GUI runs in a seperate process.  This is necessary, as the GUI requires its own mainloop, which is incompatible
with the python main loop.  When a graph is drawn, communication is through a multiprocess Queue.  Any data to be plotted
is actually serialized and sent to the gui process to be displayed.  For this reason, it is best to try to display small amounts
of data.  Anything over 32mb may be problematic.  Fortunately, I have never come close to this limit in this software.

An alternative would to use shared memory, however this would take some planning ahead.  numpy does not allocate memery by 
default in to shareable address space, so you it would be necessary to allocate shared arrays instead of normal numpy arrays.
The communication channels would also need to be modified.  However with the fast forward functionality, the performance gain
