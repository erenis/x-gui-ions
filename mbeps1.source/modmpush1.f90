...
!          calls PPMOVIN1L
! mcopyout1 copy ordered particles to unordered array
!           calls PPCOPYOUT1
...
! check for errors
      if (irc /= 0) then
         write (*,*) 'mmovin1 overflow error, irc=', irc
         stop
      endif
      end subroutine
!
!-----------------------------------------------------------------------
      subroutine mpcopyout1(part,ppart,kpic,irc)
...
! extract dimensions
      ...
      mx1 = size(kpic,1)
! call low level procedure
      call PPCOPYOUT1(part,ppart,kpic,nop,nppmx,idimp,mx1,irc)
      end subroutine
!
!-----------------------------------------------------------------------
...
! check error
      if (irc /= 0) then
         write (*,*) 'mrpushf1zf error: irc=', irc
         stop
      endif
      end subroutine
!
