...
! declare scalars for diagnostics
      integer :: it, iw, mtw, mtp, mtel, mtv, mtt
      integer :: itw, itp, itel, itv, itt
...
! open file: updates nderec and possibly iude
         if (nderec==0) call dafopennc1(denet,iude,nderec,trim(fdename))
      endif
!
...
! open file: updates ndirec and possibly iudi
         if (ndirec==0) call dafopennc1(denit,iudi,ndirec,trim(fdiname))
...
! open file: updates nprec and possibly iup
         if (nprec==0) call dafopennc1(pott,iup,nprec,trim(fpname))
! spectral analysis
...
! open file: updates nelrec and possibly iuel
         if (nelrec==0) call dafopennc1(elt,iuel,nelrec,trim(felname))
      endif
!
! initialize velocity diagnostic
      if (ntv > 0) then
         allocate(sfv(2*nmv+2,1,mx1+1),fvm(3,1))
         mtv = (nloop - 1)/ntv + 1; itv = 0
         allocate(fvtm(mtv,3,1))
         sfv(1,:,:) = 2.0*max(4.0*vtx+abs(vx0),4.0*vtdx+abs(vdx))
         fvtm = 0.0
! ions
         if (movion==1) then
            allocate(sfvi(2*nmv+2,1,mx1+1),fvmi(3,1))
            allocate(fvtmi(mtv,3,1))
...
! initialize trajectory diagnostic
      if (ntt > 0) then
         ...
         if ((nst==1).or.(nst==2)) then
            ...
            allocate(fvtp(2*nmv+2,1),fvmtp(3,1))
...
! display smoothed ion density
                  call dscaler1(sfield,' IDENSITY',ntime,999,1,nx,irc)
                  if (irc==1) exit; irc = 0
               endif
! ion spectral analysis
               if ((nddi==2).or.(nddi==3)) then
...
! display frequency spectrum
                  call dmscaler1(wkdi,'IDENSITY OMEGA VS MODE',ntime,999&
     &,1,modesxdi,cwk,irc)
                  if (irc==1) exit; irc = 0
               endif
            endif
...
! display frequency spectrum
               call dmscaler1(wk,'POTENTIAL OMEGA VS MODE',ntime,999,1, &
     &modesxp,cwk,irc)
               if (irc==1) exit; irc = 0
            endif
         endif
...
! velocity diagnostic
      if (ntv > 0) then
         ts = t0 + dt*real(ntv)
         call displayfvt1(fvtm,' ELECTRON',ts,dt*real(ntv),itv,irc)
! ions
         if (movion==1) then
...
! trajectory diagnostic
      if (ntt > 0) then
         if ((nst==1).or.(nst==2)) then
            ...
            ts = t0 + dt*real(ntt)
            call displayt1(partd,ts,dt*real(ntt),itt,2,3,irc)
            if (irc==1) stop
         endif
      endif
!
...
! performs frequency analysis of accumulated complex time series
            ...
            wkdi(:,2) = wmi(maxloc(pkwdi(:,:,2),dim=2))
! display frequency spectrum
            call dmscaler1(wkdi,'IDENSITY OMEGA VS MODE',ntime,999,1,   &
     &modesxdi,cwk,irc)
         endif
      endif
!
