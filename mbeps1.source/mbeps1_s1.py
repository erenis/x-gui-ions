...
"""
This file is step 1, in modifying the directly translated Fortran python, mbeps1.py, in to a form that can be connected dynamically to a GUI
The spirit is to store all the simulation state variables in one data structure.  In this case, we store everything in the Fortran intialized
object called in1.  in1 will contain all the namelist variables read from a namelist.  We also tack on a number of other attributes created in
init(s1).  We create three new functions init, step, and write.  Each take the state object as an argument.  This step is mostly an
excercise in renaming variables.
"""
